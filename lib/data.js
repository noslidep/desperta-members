...
export async function getMyPrograms(){
 ...
 for(const p of (programs||[])){
  ...
  if(!canAccess){
   results.push({...p,enrolled:false,access_state,access_expires_at:enrollment?.expires_at||null,total_lessons:0,completed_lessons:0,progress:0})
   continue
  }
  ...
  results.push({...p,enrolled:true,access_state:'active',access_expires_at:enrollment.expires_at,total_lessons:total||0,completed_lessons:done||0,progress:total?Math.round(((done||0)/total)*100):0})
 }
 return results
}
