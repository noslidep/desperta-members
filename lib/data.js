...
export async function getMyPrograms(){
 ...
 for(const p of (programs||[])){
  ...
  results.push({...withResolvedCover(supabase,p),enrolled:true,access_state:'active',admin_access:isAdmin,access_expires_at:isAdmin?null:enrollment?.expires_at||null,total_lessons:total||0,completed_lessons:done||0,progress:total?Math.round(((done||0)/total)*100):0})
 }
 return results
}
...
export async function getProgramBySlug(slug){
 ...
 const mapped=(modules||[]).filter(m=>m.status==='published').map(m=>({...m,lessons:(m.lessons||[]).filter(l=>l.status==='published').map(l=>({...l,is_completed:completed.has(l.id)}))}))
 const total=mapped.reduce((n,m)=>n+m.lessons.length,0),done=mapped.reduce((n,m)=>n+m.lessons.filter(l=>l.is_completed).length,0)
 return {program:{...withResolvedCover(supabase,program),admin_access:isAdmin,access_expires_at:isAdmin?null:enrollment?.expires_at||null,total_lessons:total,completed_lessons:done,progress:total?Math.round(done/total*100):0},modules:mapped}
}
...
export async function getLesson(id){
 ...
 const completed=new Set((progress||[]).filter(x=>x.completed).map(x=>x.lesson_id)); const mapped=(modules||[]).filter(m=>m.status==='published').map(m=>({...m,lessons:(m.lessons||[]).filter(l=>l.status==='published').map(l=>({...l,is_completed:completed.has(l.id)}))}))
