...
export async function getMyPrograms(){
 ...
 const [{data:programs},{data:enrollments}]=await Promise.all([
  supabase.from('programs').select('*').in('status',['published','coming_soon']).order('position'),
 ...
 for(const p of (programs||[])){
  ...
  const canAccess=current && p.status==='published'
  let access_state='locked'
  ...
  results.push({...withResolvedCover(supabase,p),enrolled:true,access_state:'active',access_expires_at:enrollment.expires_at,total_lessons:total||0,completed_lessons:done||0,progress:total?Math.round(((done||0)/total)*100):0})
 }
 return results
}
...
export async function getProgramBySlug(slug){
 ...
 return {program:{...withResolvedCover(supabase,program),access_expires_at:enrollment.expires_at,total_lessons:total,completed_lessons:done,progress:total?Math.round(done/total*100):0},modules:mapped}
}
...
export async function getLesson(id){
 ...
 return {lesson:{...lesson,is_completed:completed.has(lesson.id)},program:withResolvedCover(supabase,program),module,modules:mapped,access_expires_at:enrollment.expires_at}
}
...
export async function getMaterials(){
 ...
 const{data}=await s.from('materials').select('*,programs(title)').in('program_id',ids).eq('status','published').order('created_at',{ascending:false})
