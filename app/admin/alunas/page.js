...
function accessSummary(accesses=[]){
 ...
 if(!current.length)return 'Sem acessos ativos'
...
export default async function Students({searchParams}){
 ...
 return <div className="page">
  <div className="section-head"><div><span className="eyebrow">Acessos</span><h3 style={{fontSize:30}}>Alunas</h3><p>Convide, altere e controle exatamente quais conteúdos cada aluna pode acessar.</p></div></div>
  ...
  <div className="grid grid-2 admin-access-top">
   <form action={inviteStudent} className="card form-card">
    ...
    <p className="footer-note">A nova aluna nasce sem conteúdo liberado. O acesso é definido individualmente no painel.</p>
   </form>
   <div className="card form-card">
    <span className="eyebrow">Regra central</span><h3 style={{color:'#071b3e'}}>Acesso individual</h3>
    <p className="muted">Uma aluna pode ter um, vários ou nenhum acesso. URLs internas de módulos, aulas, materiais e eventos protegidos também respeitam a liberação ativa e a validade.</p>
  ...
  <section className="section">
   ...
   <form action={bulkUpdateAccess} className="card table-wrap">
    <div className="bulk-toolbar">
     <div className="field compact-field"><label>Conteúdo</label><select name="program_id" required defaultValue=""><option value="" disabled>Selecione...</option>{programs.map(p=><option key={p.id} value={p.id}>{p.title}</option>)}</select></div>
    ...
    <table className="table admin-students-table"><thead><tr><th style={{width:40}}>Sel.</th><th>Aluna</th><th>Acessos ativos</th><th>Cadastro</th><th></th></tr></thead><tbody>
